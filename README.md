...
提供 **Web 版**（浏览器访问）和 **桌面版**（Tauri 打包，支持 **macOS** / **Windows**）。业务逻辑集中在 TypeScript `core/` 层，UI 只负责组合与交互，便于后续扩展知乎、掘金等平台适配器。
...
**AI 助手仅桌面版可用。** 它通过 Tauri 调用本机已安装的 [Codex CLI](https://github.com/openai/codex)（`codex login` / `codex exec`），浏览器出于安全限制无法启动本地进程，因此即使用 `npm run dev` 在 `http://127.0.0.1` 访问也**不能**使用 AI 功能。请使用 `npm run tauri:dev`，或从 [Releases](https://github.com/ingeniousfrog/Read2MD-Studio/releases) 安装 **macOS .dmg** / **Windows 安装包**。
...
| 同上 | **Windows 10/11 (x64)**：`Read2MD-Studio_0.3.0_x64-setup.exe` |
...
- 适用于 **Windows 10 / 11，64 位（x64）**
- 需要系统已安装 **Microsoft Edge WebView2**（Win10/11 通常已自带）
- 双击 `Read2MD-Studio_0.3.0_x64-setup.exe` 运行 NSIS 安装向导，完成后从开始菜单或桌面快捷方式启动
...
- **中英文界面**：Toolbar 语言切换，界面文案全面 i18n
- **Codex 用量**：设置页展示 Session / Weekly 余量与 Credits
- **导出长图**：一键导出 Preview 渲染结果为 PNG
- **Mermaid 稳定性**：预览即时渲染；复制到公众号时栅格化为图片
- **公众号导入修复**：列表项丢失、display 公式空白、引用块 raw HTML 泄漏等
- **复制到公众号修复**：桌面版改用原生剪贴板 API，含 Mermaid 的长文复制更稳定
- 新增桌面端 **AI 助手**：基于本地 Codex CLI，Web 版不可用
- **Cowork 流水线**：步骤可编辑，一键执行，配置持久化
- **Windows 桌面版**：Releases 提供 x64 安装包（`.exe`）
...
**依赖：** [Rust](https://www.rust-lang.org/tools/install) 1.77+；macOS 另需 Xcode Command Line Tools
...
当前版本：**0.3.0**。预构建安装包见 [Releases](https://github.com/ingeniousfrog/Read2MD-Studio/releases)（macOS dmg / Windows exe）。

**AI 助手：** 需本机已安装 Codex CLI（`codex --version` 可检查）。首次使用：工具栏 **设置** → **登录 Codex**（浏览器 OAuth）；设置页显示绿点「已登录」后即可使用。Preview 页眉 **AI 助手** 打开能力面板；可在设置中为不同能力单独选择模型。macOS 与 Windows 桌面版均支持。
...
| `npm run tauri:build` | 构建桌面安装包（macOS：dmg；Windows：exe） |
...
- macOS 桌面版为 ad-hoc 签名、未公证；Windows 预构建安装包见 [Releases](https://github.com/ingeniousfrog/Read2MD-Studio/releases)
